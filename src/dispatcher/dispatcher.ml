...
let run_promise ~sw { stream; terminating = term, _; _ } ~f =
  if Promise.is_resolved term then raise (Invalid_argument "Workpool.terminate already called");

  let p1, w1 = Promise.create () in
  Fiber.fork_promise ~sw (fun () ->
    Eio.Stream.add stream (Process (Pack (f, w1)));
    Promise.await_exn p1 )

let run { stream; terminating = term, _; _ } ~f =
  if Promise.is_resolved term then raise (Invalid_argument "Workpool.terminate already called");
